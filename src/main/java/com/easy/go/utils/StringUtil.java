...
public class StringUtil {
    ...
    public final static String regex = "'|#|%|;|--| and | and|and |and| or | or|or |or| not | not|not|not " +
            "| use | use|use |use| insert | insert|insert |insert| delete | delete|delete |delete| update | update|update |update" +
            "| select | select|select |select| count | count|count |count| group | group|group |group| union | union|union |union" +
            "| create | create|create |create| drop | drop|drop |grop| truncate | truncate|truncate |truncate| alter | alter|alter |alter" +
            "| grant | grant|grant |grant| execute | execute|execute |execute| exec | exec|exec |exec| xp_cmdshell | xp_cmdshell|xp_cmdshell |xp_cmdshell" +
            "| call | call|call |call| declare | declare|declare |declare| source | source|source |source| sql | sql|sql |sql";
